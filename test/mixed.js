...
test(`data range undefined`, async (t) => {
  const result = evaluate(q2, {});
  t.false(result);
});

test(`timezone`, async (t) => {
  const operators1 = getOperators({ timezone: 'fantasyLand' });
  const operators2 = getOperators();
  t.deepEqual(operators1._tz, 'fantasyLand');
  t.deepEqual(operators2._tz, undefined);
});
